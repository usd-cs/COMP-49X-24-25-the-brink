...
test('redirects to Home page when Home button is clicked', async () => {
  await act(async () => {
    render(
      <MemoryRouter initialEntries={['/profile']}>
        <App />
  ...
  const homeButton = await screen.findByRole('button', { name: /Edit Profile/i });
  fireEvent.click(homeButton);

  const competitionHeading = await screen.findByText(/Competitions/i);
  expect(competitionHeading).toBeInTheDocument();
});
